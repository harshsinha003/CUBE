...
class Controls {
  ...
  initDraggable() {
    ...
    this.draggable.onDragMove = position => {
      ...
      if ( this.state === PREPARING && this.dragTotal.length() > 0.03 ) {
        ...
        if ( this.flipType === 'layer' ) { 
          ...
          this.group.rotateOnAxis( this.flipAxis, rotation * 1.8 );
          this.flipAngle += rotation * 1.8;

        } else {

          this.edges.rotateOnWorldAxis( this.flipAxis, rotation * 1.8 );
    ...
    this.draggable.onDragEnd = position => {
      ...
      const flip = ( Math.abs( momentum ) > 0.03 && Math.abs( this.flipAngle ) < Math.PI / 2 );
  ...
  constructor( game ) {
    ...
    this.defaults = {
      cube: {
        ...
        L: 0x82ca38, // green
        P: 0x08101a, // piece
        G: 0x0f0f23, // background - dark theme
      },
      erno: {
        U: 0xffffff,
        D: 0xffd500,
